...
country_list = list(set(df.query("UNRegion=='Asia'")["country"]))
    ...
    children = [
        html.Div(
            ...
            [
                ...
                dbc.CardBody(html.Div(id="card-content-2", className="mb-3")),
...
@app.callback(
    Output("card-content-2", "children", allow_duplicate=True), [Input("card-tabs", "active_tab")],prevent_initial_call=True
)
...
@app.callback(
    Output("graph-op2", "children", allow_duplicate=True), Input("submit-button-state", "n_clicks"), 
    State('country_checkbox', 'value'), State('chart_checkbox', 'value'), prevent_initial_call=True
)
