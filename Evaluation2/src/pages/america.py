...
import graphs
import tabfunc_america as tf
import dash_mantine_components as dmc
import dash_bootstrap_components as dbc
from index import app
from dash.dependencies import Input, Output, State

country_list = ['Antigua and Barbuda', 'Argentina', 'Bahamas', 'Barbados','Bermuda', 'Bolivia', 'Brazil', 'Aruba', 'Belize', 'Canada','Cayman Islands', 'Chile', 'Colombia', 'Costa Rica', 'Cuba','Dominica', 'Dominican Republic', 'Ecuador', 'El Salvador','French Guiana', 'Grenada', 'Guadeloupe', 'Guatemala', 'Guyana','Haiti', 'Honduras', 'Jamaica', 'Martinique', 'Mexico','Montserrat', 'Nicaragua', 'Panama', 'Paraguay', 'Peru','Saint Lucia', 'Suriname', 'Trinidad and Tobago','United States of America', 'Uruguay','Venezuela, Bolivarian Republic of']

t1_chart_radiobuttons = ["Region-wise distribution", "Population-GDP Correlation"]
t2_chart_radiobuttons = ["Basic Population Analysis", "Population Growth Analysis", "Population Value Table", "Population Growth Rate"]
t3_chart_radiobuttons = ["GDP per Capita Trend", "GDP per Capita Value"]
t4_chart_radiobuttons = ["Country-wise Trend for EF vs BC", "Entire UN Region EF vs BC Analysis", "Country-wise Deficit vs Reserve"]
    ...
    children = [
        html.Div(
            ...
            [
                ...
                dbc.CardBody(html.Div(id="card-content-4", className="mb-3")),
...
@app.callback(
    Output("card-content-4", "children", allow_duplicate=True), [Input("card-tabs", "active_tab")], prevent_initial_call=True,
)
...
@app.callback(
    Output("graph-op4", "children", allow_duplicate=True), Input("submit-button-state", "n_clicks"), State('country_checkbox', 'value'), State('chart_checkbox', 'value'), prevent_initial_call=True,
